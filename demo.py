...
target, input, depth, ato = target.to(device), input.to(device), depth.to(device), ato.to(device)

val_target, val_input, val_depth, val_ato = val_target.to(device), val_input.to(device), val_depth.to(device), val_ato.to(device)
...
label_d = Variable(label_d.to(device))
...
ssimall=0
iteration = 0
nameOfFile = glob.glob('testDCPDNIP\\*h5')
#nameOfFile = nameOfFile[40:]
root = 'testDCPDNIP\\'
# print(1)
...
for epoch in range(1):
  for i, data in enumerate(valDataloader, 0):
    ...
    #print(data[:-1])
    if opt.mode == 'B2A':
        input_cpu, target_cpu, depth_cpu, ato_cpu = data[:-1]
    elif opt.mode == 'A2B' :
        input_cpu, target_cpu, depth_cpu, ato_cpu = data[:-1]
    batch_size = target_cpu.size(0)
    # print(i)
    
    target_cpu, input_cpu, depth_cpu, ato_cpu = target_cpu.float().to(device), input_cpu.float().to(device), depth_cpu.float().to(device), ato_cpu.float().to(device)
    
    # get paired data
    #***********changes - added with torch.no_grad() and data.resize_as_ to .resize
    with torch.no_grad():   
    ...
    index2 = 0
    directory='testDCPDNOP'
    if not os.path.exists(directory):
        os.makedirs(directory)
    for i in range(opt.valBatchSize):
        nameTogive = nameOfFile[index]
        index=index+1
        print(index)
        zz1=zz[index2,:,:,:]
        #print(type(zz1))
        vutils.save_image(zz1, 'testDCPDNOP\\'+nameTogive[len(root):-3]+'_DCPCN.jpg', normalize=True, scale_each=False)
trainLogger.close()
