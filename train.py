...
target, input, trans, ato = target.to(device), input.to(device), trans.to(device), ato.to(device)
val_target, val_input, val_trans, val_ato = val_target.to(device), val_input.to(device), val_trans.to(device), val_ato.to(device)
...
#trying not to initilise vgg model
...
val_tran_cpu, val_ato_cpu = val_tran_cpu.float().to(device), val_ato_cpu.float().to(device)
...
for epoch in range(opt.niter):
  ...
  for i, data in enumerate(dataloader, 0):
    ...
    target_cpu, input_cpu, trans_cpu, ato_cpu = target_cpu.float().to(device), input_cpu.float().to(device), trans_cpu.float().to(device), ato_cpu.float().to(device)
